...
- **교리 갈래는 하나의 정답으로 봉합하지 않는다.** 서로 다른 전통의 답을 입장별로 나란히 세우고
  ...
  "둘 다 맞다"로 얼버무리지도 않는다.
- 갈래별 신학자 패널은 임의 배치가 아니라 **질문의 목적에 따라 달라지는 선정 기준**의 결과다.
  고민(5인) = 좋은 답변자 조합 / 삶(5인) = 상담자 조합 / 교리(6인) = 전통 대표성 극대화.
- **교리 갈래의 아우구스티누스는 "초대교회 전체"가 아니라 "서방 라틴 교부"의 기준점**이다.
  동방은 원죄·예정에서 그와 갈라지며, 그 차이는 다마스쿠스의 요한이 드러낸다.
  패널 인원은 `api/ask.js`의 `PANELS`와 `assets/data.js`의 `DATA[cat].panel`을 함께 고쳐야 하며,
  두 곳의 **순서와 이름이 정확히 일치**해야 한다(프론트엔드가 순서로 초상을 매칭한다).
...
**Windows Git Bash에서 `curl`로 한글 질문을 보내지 마십시오.** 본문이 CP949로 인코딩되어
서버에는 깨진 문자가 도착하고, AI는 알아볼 수 없는 입력을 받아 엉뚱한 주제로 답합니다
(실제로 이 때문에 없는 버그를 쫓은 적이 있습니다). 한글 API 테스트는 **브라우저에서
`fetch`로** 하거나, UTF-8로 저장한 파일을 `curl --data-binary @파일`로 보내십시오.
